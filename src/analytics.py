class ReportGenerator:
    ...
    def get_wait_times(self, queue):
        ...
        # queue.size() is a method but sometimes used as property in his diff
        size = queue.size() if callable(queue.size) else queue.size
        minutes = 14 if size > 0 else 0
    ...
    def search_patient_history(self, patient_id, all_patients):
        ...
        while current:
            # His diff used .value, but I use .data
